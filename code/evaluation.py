...
def convert_to_list(y_aspect, y_opinion, y_sentiment, mask):
    y_aspect_list = []
    y_opinion_list = []
    y_sentiment_list = []
    for seq_aspect, seq_opinion, seq_sentiment, seq_mask in zip(y_aspect, y_opinion, y_sentiment, mask):
        l_a = []
        l_o = []
        l_s = []
        for label_dist_a, label_dist_o, label_dist_s, m in zip(seq_aspect, seq_opinion, seq_sentiment, seq_mask):
            if m == 0:
                break
            else:
...
def get_metric(y_true_aspect, y_predict_aspect, y_predict_opinion, y_true_opinion, y_true_sentiment, y_predict_sentiment, mask, train_op):
    f_a, f_o = 0, 0
    true_aspect, true_opinion, true_sentiment = convert_to_list(y_true_aspect, y_true_opinion, y_true_sentiment, mask)
    predict_aspect, predict_opinion, predict_sentiment = convert_to_list(y_predict_aspect, y_predict_opinion, y_predict_sentiment, mask)
    
    f_aspect, acc_s, f_s, f_absa = score(true_aspect, predict_aspect, true_sentiment, predict_sentiment, 0)
    f_opinion, _, _, _ = score(true_opinion, predict_opinion,  true_sentiment, predict_sentiment, 0)
    #f_opinion,
